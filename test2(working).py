...
--Onehot encode the label columns
--FIlter data based on each paramater
--Take 70% of data after filtering, for each paramater
--Append this 70% to a new df
--Append the 30% to a new df

'''

df = pd.get_dummies(df, columns=['Class'])
df.dropna(inplace=True)
#df.to_excel('SensorReadings24.xlsx', sheet_name='Data_Onehot')
...
train_data_MF,test_data_MF = split_data(df_MF)
train_data_SIRT,test_data_SIRT = split_data(df_SIRT)
train_data_SHRT,test_data_SHRT = split_data(df_SHRT)
train_data_SILT,test_data_SILT = split_data(df_SILT)


Test = pd.DataFrame()
Train = pd.DataFrame()

Test = pd.concat([test_data_MF,test_data_SIRT,test_data_SHRT,test_data_SILT])
Train = pd.concat([train_data_MF,train_data_SIRT,train_data_SHRT,train_data_SILT])


data_test = np.array(Test).T
data_train = np.array(Train).T

#maybe do a shuffle here
...
W1, b1, W2, b2 = gradient_descent(X_train, Y_train, 0.15, 10000)
